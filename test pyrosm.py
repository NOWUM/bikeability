...
# Defaultwerte für Wege-Scores
DEFAULT = {'separation': 2,
           'surface': 2}

#TODO: move rightofway back out of the separate scoring

def score_route_separation(network_osm: pd.DataFrame, scoring: pd.DataFrame):
...
def score_route_surfaces(network_osm: pd.DataFrame, scoring: pd.DataFrame):
...
def score_osm():
    ...
    fp = "C:\\Users\\jk2932e\\Python Projects\\Masterarbeit\\pyrosm\\Aachen.osm.pbf"
    osm = pyrosm.OSM(fp)
    ...
    scoring, missing_scores = score_route_separation(network_osm = network_osm,
                                                     scoring = scoring)
    
    log.info("Successfully scored for separation. Starting to score for surface area!")
    scoring, missing_scores = score_route_surfaces(network_osm = network_osm,
                                                   scoring = scoring)
    return scoring, missing_scores
...
scoring, missing_scores = score_osm()
